...
def run_cnn(config, frame_spec_label_train, frame_spec_label_test, predictions, accuracy, ph_action, ph_label, ph_train):
    ...
    # Return The Train and Test Accuracies
    return total_train_acc, total_test_acc, prediction
...
def run_cnn_wout_epochs(max_epochs, fsl_train, fsl_test):
    ...
    for n in range(max_epochs):
        ...
        with tf.Graph().as_default():
            ...
            train_acc, test_acc, predicts = run_cnn(config, fsl_train, fsl_test, predictions, accuracy, ph_action, ph_label, ph_train)
            sensitivity, specificity = calc_metrics(fsl_test[:, 4], predicts)
#            if ((train_acc > 0.90)  or (test_acc > 0.90)):
#                break
    return None

def run_cnn_epochs(train_epochs, fsl_train, fsl_test):
...
def do_cnn():
    # Function to Execute CNN Program
    
    # Get Datasets
    print ('~' * 90)
    train_data, test_data = get_dataset_splits()
    ...
    # Get Predictions
    test_preds = run_cnn_wout_epochs(1, fsl_train, fsl_test)
    #test_preds = run_cnn_epochs(3, fsl_train, fsl_test)
    print ('~' * 90)
