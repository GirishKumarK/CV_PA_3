...
def get_video_specs(video_file_name):
    ...
    # Normalizing All Video Frame Sizes to h90 x w120
    video = skvio.vread(video_file, as_grey=True, outputdict={"-sws_flags": "bilinear", "-s": "120x90"}) # -s : width x height
...
def do_svm(kernel, train_data, valid_data, test_data, train_labels, valid_labels, test_labels):
    # Convert Datset to Numpy Array
    train_data = np.array(train_data)
    valid_data = np.array(valid_data)
    test_data = np.array(test_data)
    train_labels = np.array([trl for trl in train_labels]).ravel()
    valid_labels = np.array([vl for vl in valid_labels]).ravel()
    test_labels = np.array([tl for tl in test_labels]).ravel()
    # Kernel Constants
    lin_c = 1.0
    rbf_c, rbf_gamma = 1.0, 2.0
    ...
    if (kernel == 'linear'):
        ...
        # Validate
        t = time.time()
        y_lin = svc_lin.predict(valid_data)
        svc_predict = time.time() - t
        print('Time Taken to Predict using the Fitted Model : ' + str(svc_predict) + ' Secs')
        acc_lin = r2_score(valid_labels, y_lin) * 100
        print('Validation Accuracy Score of the Model : ' + str(acc_lin))
        # Test
        t = time.time()
        ...
        print('\nGaussian Kernel with Hyperparameters : C = ' + str(rbf_c) + 
              ', Gamma = ' + str(rbf_gamma))
        # Train
        svc_rbf = svm.SVC(kernel='rbf', C=rbf_c, gamma=rbf_gamma, cache_size=4096)
        t = time.time()
        ...
        # Validate
        t = time.time()
        y_rbf = svc_rbf.predict(valid_data)
        svc_predict = time.time() - t
        print('Time Taken to Predict using the Fitted Model : ' + str(svc_predict) + ' Secs')
        acc_rbf = r2_score(valid_labels, y_rbf) * 100
        print('Validation Accuracy Score of the Model : ' + str(acc_rbf))
        # Test
        t = time.time()
        ...
        # Train
        svc_sgm = svm.SVC(kernel='sigmoid', C=sgm_c, gamma=sgm_gamma, coef0=sgm_coef, cache_size=4096)
        t = time.time()
        ...
        # Validate
        t = time.time()
        y_sgm = svc_sgm.predict(valid_data)
        svc_predict = time.time() - t
        print('Time Taken to Predict using the Fitted Model : ' + str(svc_predict) + ' Secs')
        acc_sgm = r2_score(valid_labels, y_sgm) * 100
        print('Validation Accuracy Score of the Model : ' + str(acc_sgm))
        # Test
        t = time.time()
        ...
        # Validate
        t = time.time()
        y_poly = svc_poly.predict(valid_data)
        svc_predict = time.time() - t
        print('Time Taken to Predict using the Fitted Model : ' + str(svc_predict) + ' Secs')
        acc_poly = r2_score(valid_labels, y_poly) * 100
        print('Validation Accuracy Score of the Model : ' + str(acc_poly))
        # Test
        t = time.time()
...
print ('~' * 90)
train, valid, test = get_dataset_splits()
print ('~' * 90)
fsl_train = get_videos_as_images_with_labels(train, 'Train') # ~4.00 Mins
fsl_valid = get_videos_as_images_with_labels(valid, 'Validation') # ~0.50 Mins
fsl_test = get_videos_as_images_with_labels(test, 'Test') # ~0.50 Mins
print ('~' * 90)
hog_train = get_hog_features(fsl_train[:, 0], 'Train') # ~8.00 Mins
hog_valid = get_hog_features(fsl_valid[:, 0], 'Validation') # ~1.00 Mins
hog_test = get_hog_features(fsl_test[:, 0], 'Test') # ~1.00 Mins
print ('~' * 90)
lin_labels, lin_preds = do_svm('linear', hog_train, hog_valid, hog_test, fsl_train[:, 4], fsl_valid[:, 4], fsl_test[:, 4])
lin_sens, lin_spec = calc_metrics(lin_labels, lin_preds)
rbf_labels, rbf_preds = do_svm('gaussian', hog_train, hog_valid, hog_test, fsl_train[:, 4], fsl_valid[:, 4], fsl_test[:, 4])
rbf_sens, rbf_spec = calc_metrics(rbf_labels, rbf_preds)
sgm_labels, sgm_preds = do_svm('sigmoid', hog_train, hog_valid, hog_test, fsl_train[:, 4], fsl_valid[:, 4], fsl_test[:, 4])
sgm_sens, sgm_spec = calc_metrics(sgm_labels, sgm_preds)
poly_labels, poly_preds = do_svm('poly', hog_train, hog_valid, hog_test, fsl_train[:, 4], fsl_valid[:, 4], fsl_test[:, 4])
